...
class AES(object):
    ...
    aes_new = AES(mode='ecb', input_type='hex')
            
    # Test vector 128-bit key
    key = '000102030405060708090a0b0c0d0e0f'
        
    # Encrypt data with your key
    cyphertext = aes_new.encryption('00112233445566778899aabbccddeeff', key)
        
    # Decrypt data with the same key
    plaintext = aes_new.decryption(cyphertext, key)
    """
    ...
    def cbc(self, data, expanded_key, isInv):
        ...
        if self.iv is None: self.iv = "00000000000000000000000000000000"
...
def encrypt(mode,data,key):
    aes_new = AES(mode=mode, input_type='hex')
    # Encrypt data with your key
    cyphertext = aes_new.encryption(data, key)
    return cyphertext

def decrypt(mode,data,key):
    aes_new = AES(mode=mode, input_type='hex')
    plaintext = aes_new.decryption(data, key)
    return plaintext
