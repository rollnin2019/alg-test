...
def _line_conv_L(srcdata):
    ...
    return srcdata ^ common.lrotate(srcdata, 2) ^ common.lrotate(srcdata, 10) ^ common.lrotate(srcdata,
                                                                                               18) ^ common.lrotate(
        srcdata, 24)
...
# print(C)
# sm4 = SM4(key='404142434445464748494A4B4C4D4E4F')
# res = sm4.sm4_decrypt(C,SM4_CBC)
# print(res)

def encrypt(mode,data,key):
    sm4 = SM4(key=key)
    if mode == "cbc":
        C = sm4.sm4_encrypt(message_padding(data, SM4_PAD_UNION), SM4_CBC)
    elif mode == 'ecb':
        C = sm4.sm4_encrypt(message_padding(data, SM4_PAD_UNION), SM4_ECB)
    else:
...
def decrypt(mode,data,key):
    sm4 = SM4(key=key)
    if mode == "cbc":
        res = sm4.sm4_decrypt(data, SM4_CBC)
    elif mode == 'ecb':
        res = sm4.sm4_decrypt(data, SM4_ECB)
    else:
        raise Exception("Wrong mode")
    return res
